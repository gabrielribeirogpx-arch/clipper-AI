...
class SubtitleRenderer:
    ...
    def safe_caption_y(self, video_h: int) -> int:
        margin = max(int(video_h * self.config.safe_bottom_ratio), self.config.min_safe_bottom_px)
        return max(0, video_h - margin)
    ...
    def build_word_layers(self, words: List[Dict], video_w: int, video_h: int) -> List[TextClip]:
        clips: List[TextClip] = []
        max_caption_w = int(video_w * 0.84)
        base_y = self.safe_caption_y(video_h)
        ...
        for word_data in words:
            ...
            font_size = self.config.emphasis_font_size if emphasis else self.config.normal_font_size
            ...
            shadow = (
                TextClip(word, fontsize=font_size, font=font_name, color="#000000", kerning=-2, method="caption", size=(max_caption_w, None), align="center", stroke_color="#111111", stroke_width=stroke_width + 3)
            ...
            glow = (
                TextClip(word, fontsize=font_size, font=font_name, color=glow_color, kerning=-2, method="caption", size=(max_caption_w, None), align="center", stroke_color=glow_color, stroke_width=stroke_width + 2)
            ...
            text = (
                TextClip(word, fontsize=font_size, font=font_name, color=fill_color, stroke_color="#0D0D0D", stroke_width=stroke_width, kerning=-2, method="caption", size=(max_caption_w, None), align="center")
