...
export function SequencesView({ onOpenTimeline }: { onOpenTimeline: () => void }) {
  return <motion.section initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={springConfigs.smooth} className="panel-premium h-full min-h-0 overflow-x-auto p-4"><p className="panel-title">Generated sequences</p><div className="flex h-[calc(100%-1.6rem)] gap-3">{clips.map((clip) => <article key={clip.title} className="group flex h-full w-56 flex-shrink-0 flex-col rounded-2xl border border-white/10 bg-slate-950/60 p-2"><div className="relative flex-1 rounded-xl bg-gradient-to-b from-slate-600 to-black"><div className="absolute inset-0 hidden place-items-center rounded-xl bg-cyan-400/12 text-xs text-cyan-100 group-hover:grid">Autoplay preview</div></div><h3 className="mt-2 line-clamp-2 text-sm">{clip.title}</h3><div className="mt-2 flex gap-1 text-[11px]"><span className="premium-chip px-1.5 py-0.5">Viral {clip.viral}</span><span className="premium-chip px-1.5 py-0.5">Ret {clip.retention}</span><span className="premium-chip px-1.5 py-0.5">Hook {clip.hook}</span></div><div className="mt-2 flex flex-wrap gap-1">{clip.tags.map((tag) => <span key={tag} className="rounded-full border border-white/10 px-2 py-0.5 text-[10px] text-slate-300">{tag}</span>)}</div><div className="mt-2 grid grid-cols-3 gap-1 text-[11px]"><button className="rounded-md border border-white/10 py-1">Export</button><button className="rounded-md border border-white/10 py-1">Duplicate</button><button onClick={onOpenTimeline} className="rounded-md border border-cyan-300/35 py-1 text-cyan-200">Timeline</button></div></article>)}</div></motion.section>;
}
