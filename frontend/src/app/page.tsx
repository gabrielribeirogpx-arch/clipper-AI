...
export default function Home() {
  ...
  return (
    <main className="relative min-h-screen overflow-hidden bg-[var(--bg-primary)] text-slate-100">
      ...
      <div className="relative mx-auto grid min-h-screen w-full max-w-[2200px] grid-cols-1 gap-5 px-5 py-5 2xl:grid-cols-[290px_minmax(0,1fr)_420px]">
        <aside className="panel-premium flex min-h-[94vh] flex-col p-6">
          ...
          <nav className="mt-6 flex-1 space-y-2">
            {navItems.map((item) => <motion.button key={item.label} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} transition={springConfigs.snappy} className={`flex w-full items-center gap-3 rounded-xl border px-4 py-3 text-left text-sm transition ${item.active ? 'border-cyan-300/35 bg-cyan-400/10 text-cyan-100 shadow-[0_0_0_1px_rgba(56,189,248,.22)]' : 'border-white/5 bg-white/[0.02] text-slate-300 hover:border-white/15 hover:bg-white/[0.04]'}`}><span>{item.icon}</span><span>{item.label}</span></motion.button>)}
          </nav>
          <div className="mt-6 rounded-2xl border border-violet-300/25 bg-gradient-to-br from-violet-500/14 to-cyan-400/8 p-4"><p className="text-xs uppercase tracking-[0.17em] text-violet-200">Pro Plan</p><p className="mt-2 text-sm text-slate-300">8K exports · team review · AI scenes</p></div>
